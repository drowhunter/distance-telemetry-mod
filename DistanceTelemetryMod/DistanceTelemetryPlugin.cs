...
namespace com.drowmods.DistanceTelemetryMod
{
    ...
    public class DistanceTelemetryPlugin : BaseUnityPlugin
    {
        ...
        private void FixedUpdate()
        {
            ...
            data.Yaw = Maths.Maths.HemiCircle(_yaw * Mathf.Rad2Deg % 360);

            data.Pitch = Maths.Maths.CopySign(Vector3.Angle(new Vector3(cRigidbody.transform.forward.x, 0, cRigidbody.transform.forward.z), cRigidbody.transform.forward), cRigidbody.transform.forward.y);

            data.Roll = Maths.Maths.CopySign(Vector3.Angle(new Vector3(cRigidbody.transform.right.x  , 0, cRigidbody.transform.right.z  ), cRigidbody.transform.right),   cRigidbody.transform.right.y);
